...
async fn main() -> std::io::Result<()> {
    ...
    // concaténation de l'ip et du port pour bind le serveur dessus
    let port_ecoute = args.listen_port.to_string();
    
    let mut addr_ecoute = args.listen_addr.to_string();
    addr_ecoute.push(':');
    addr_ecoute.push_str(&port_ecoute);
    addr_ecoute.to_string();


    //let port_ecoute = args.listen_port.to_string();
    //let addr_ecoute = args.listen_addr + ":" + port_ecoute;
    ...
    let forward_url = format!("https://{forward_socket_addr}");
    ...
    .bind_rustls(addr_ecoute, config)?
    .workers(2)
    .run()
    .await
}
