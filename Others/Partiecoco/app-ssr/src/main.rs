...
async fn showthis(form_data: web::Form<FormData>) -> Result<HttpResponse, MyError>  { //fonction pour afficher le 2éme rendu html
    let html = Show{ thing_to_show: form_data.thing_to_show.to_string(),thing_to_show2: form_data.thing_to_show2.to_string() }.render().unwrap();
    println!("{}",html.thing_to_show);
    Ok(HttpResponse::Ok().content_type("text/html").body(html))
